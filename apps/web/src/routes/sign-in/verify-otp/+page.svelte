<script lang="ts">
	...
	let email = $state<string>(page.state.email ?? '');
	let otpId = $state<string>(page.state.otpId ?? '');
	const timer = useCountdownTimer(180);

	// return to sign-in if email or otpId is not provided
	onMount(() => {
		if (!email || !otpId) {
			goto('/sign-in');
		}
	...
	const { form, errors, enhance, submitting } = superForm<VerifyOtpForm>(
		{ otp: '' },
		{
			...
			onUpdated: async ({ form }) => {
				...
				try {
					await pb.collection('users').authWithOTP(otpId, form.data.otp);
					goto('/');
				} catch {
